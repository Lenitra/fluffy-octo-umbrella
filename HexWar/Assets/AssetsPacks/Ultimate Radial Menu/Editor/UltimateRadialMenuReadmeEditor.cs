...
public class UltimateRadialMenuReadmeEditor : Editor
{
	...
	EndPageComment[] endPageComments = new EndPageComment[]
	{
		...
		new EndPageComment()
		{
			...
			url = "https://www.tankandhealerstudio.com/ultimate-radial-sub-menu.html"
		},
		new EndPageComment()
		{
	...
	void MainPage ()
	{
		...
		if( GUILayout.Button( $"  • Check out the <b><color=#{linkColor}>Online Documentation</color></b> for the Ultimate Radial Menu!", paragraphStyle ) )
		{
		...
		if( GUILayout.Button( $"  • Check out the local <b><color=#{linkColor}>Documentation</color></b> section!", paragraphStyle ) )
			NavigateForward( 3 );
		rect = GUILayoutUtility.GetLastRect();
		EditorGUIUtility.AddCursorRect( rect, MouseCursor.Link );
